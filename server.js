...
app.get('/e/:slug', async (req, res) => {
  try {
    const event = await Event.findOne({ slug: req.params.slug }).populate('userId');
    if (!event) return res.status(404).send('The event link provided does not exist.');
...
app.listen(PORT, () => console.log(`Application actively streaming on http://localhost:${PORT}`));